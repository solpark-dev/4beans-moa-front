          ...
          >
            <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-6 ${
              theme === "pop"
                ...
                : `${currentTheme.accentBg} bg-opacity-10 ${currentTheme.accentText}`
              }`}>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            ...
            >
              ...
              <div className={`flex items-center gap-4 p-5 rounded-2xl border bg-gradient-to-br ${theme === "pop" ? "from-pink-50 to-pink-100/50" : `${currentTheme.accentBg}/5 to-[#00d4ff]/5`} ${currentTheme.filterBorder}`} style={{ borderColor: `${accentColor}20` }}>
                <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-sm">
                  <Sparkles className={`w-6 h-6 ${currentTheme.accentText}`} />
                </div>
                <div className="flex-1">
                  <p className="font-bold text-gray-900 text-lg">{selectedProduct.productName}</p>
                  <p className={`font-semibold ${currentTheme.accentText}`}>
              ...
              <div className={`space-y-8 rounded-2xl p-6 border ${theme === "dark" ? "bg-[#1E293B] border-gray-700" : theme === "christmas" ? "bg-white border-gray-200" : "bg-white border-gray-100"
                ...
                <div id="maxMembers">
                  <label className={`block text-sm font-bold mb-4 ${currentTheme.text}`}>
                    <span className="flex items-center gap-2">
                      <Users className={`w-5 h-5 ${currentTheme.accentText}`} />
                      파티 인원
                    </span>
                  </label>
                      ...
                      return (
                        <button
                          ...
                          className={`relative p-4 rounded-xl border-2 transition-all ${
                            isSelected
                              ? `${currentTheme.accentText} bg-opacity-10 shadow-md`
                              : `${currentTheme.filterBorder} bg-white hover:bg-opacity-50`
                            }`}
                          style={{
                            borderColor: isSelected ? accentColor : undefined,
                            backgroundColor: isSelected ? `${accentColor}10` : undefined,
                            boxShadow: isSelected ? `0 4px 6px -1px ${accentColor}20` : undefined
                          }}
                        >
                          ...
                          {isSelected && (
                            <motion.div
                              initial={{ scale: 0 }}
                              animate={{ scale: 1 }}
                              className={`absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center shadow-lg ${currentTheme.accentBg}`}
                              style={{ boxShadow: `0 4px 6px -1px ${accentColor}40` }}
                            >
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full bg-gray-50 border-2 p-4 rounded-xl focus:ring-2 outline-none text-gray-900 transition-all font-semibold ${
                        errors.startDate
                          ? "border-red-300"
                          : `border-gray-200 focus:border-[color:var(--theme-primary)] focus:ring-[color:var(--theme-primary)]/20`
                        }`}
                      style={!errors.startDate ? { borderColor: 'transparent' } : {}}
                    />
                    {!dates.startDate && (
                ...
                <div id="months">
                  ...
                  <div className="text-center mb-6">
                    ...
                    >
                      <span className={`text-5xl font-bold ${currentTheme.accentText}`}>{dates.months}</span>
                  ...
                  <div className="px-2 mb-4">
                    <input
                      ...
                      className="w-full h-2 bg-gray-200 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-lg [&::-webkit-slider-thumb]:transition-all [&::-moz-range-thumb]:w-6 [&::-moz-range-thumb]:h-6 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:shadow-lg"
                      style={{
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${
                          dates.months === month
                            ...
                            : `bg-gray-100 text-gray-600 hover:${currentTheme.accentText} hover:bg-opacity-10`
                          }`}
                        style={dates.months === month ? {
                ...
                {dates.startDate && dates.endDate && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className={`p-5 rounded-xl border bg-gradient-to-r ${theme === "pop" ? "from-pink-50 to-pink-100/50" : `${currentTheme.accentBg}/5 to-[#00d4ff]/5`} ${currentTheme.filterBorder}`}
          ...
          {step === 4 && (
            ...
            >
              <div className="text-center mb-8">
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 border ${currentTheme.filterBorder} ${currentTheme.accentBg} bg-opacity-10`} style={{ borderColor: `${accentColor}33` }}>
                  <Check className={`w-8 h-8 ${currentTheme.accentText}`} />
                </div>
                <h2 className="text-3xl font-bold text-gray-900 mb-2">결제 완료!</h2>
                <p className="text-gray-500">공유할 계정 정보를 입력해주세요</p>
              </div>

              <div className={`bg-white border rounded-2xl p-6 space-y-4 ${currentTheme.filterBorder}`}>
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-gray-50 border p-4 rounded-xl focus:ring-2 outline-none text-gray-900 ${currentTheme.filterBorder}`}
                    style={{ borderColor: 'transparent' }} // Simplified as reusing classes might need adjustments, but keeping it simple for now
                    required
                  />
                </div>
                <div>
              ...
              <RippleButton
                type="submit"
                className={`w-full py-4 text-white rounded-full font-bold shadow-lg transition-all transform hover:scale-[1.01] active:scale-[0.99] ${currentTheme.accentBg} ${currentTheme.hoverAccentBg}`}
                style={{ boxShadow: `0 10px 15px -3px ${accentColor}40` }}
              >
